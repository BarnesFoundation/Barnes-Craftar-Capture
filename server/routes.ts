...
const esClient = new ElasticSearch.Client({
    hosts: [
        `https://${Config.esUsername}:${Config.esPassword}@${
            Config.esHost
        }:9243`,
    ],
});
...
ApiRouter.use("/search-by-invno", async (request, response) => {
    ...
    const elasticSearchResponse = (await esClient.search({
        index: "collection",
        body: {
            size: 1,
            query: {
    ...
    if (elasticSearchResponse.length > 0) {
        const { id } = elasticSearchResponse[0]
            ._source as ElasticSearchResponse;
