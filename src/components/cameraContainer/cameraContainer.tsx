...
import {
  ...
  ClearCapturedPhoto,
  UpdatePhotoLoaded,
} from "../../store/actions/cameraActions";
...
class CameraContainer extends React.Component<
  ConnectedProps & CameraState & CollectionItemState
> {
  ...
  componentWillUnmount = () => {
    this.props.dispatch(new ClearCapturedPhoto());
  };
  ...
  onTakePhoto = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    this.props.dispatch(
      new UpdatePhotoLoaded({ photoIsLoading, photoIsLoaded })
    );

    // Get the file and convert to correctly oriented uri
    const fileUrl = URL.createObjectURL(event.target.files[0]);
    const orientedPhotoUri = (await this.resizeService.correctImageOrientation(
      fileUrl
    )) as string;
    const capturedPhotoUri = (await this.resizeService.resizeImage(
      orientedPhotoUri,
      "URI",
      "DISPLAY_IMAGE"
    )) as string;
...
export const ConnectedCameraContainer =
  connect(mapStateToProps)(CameraContainer);
