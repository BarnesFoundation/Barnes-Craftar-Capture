...
class ImageSearchView extends React.Component<Props, object> {
  ...
  public render() {
    ...
    if (requestComplete && !error && response.success) {
      ...
      setItem = (
        <Button variant="contained" onClick={this.setItem}>
          Set Item
        </Button>
      );
      ...
      successSection = (
        <div>
          ...
          <p>Record Id: {id}</p>
          <p>
            Use 'Set Item' to capture additional reference images for this item
          </p>
        </div>
      );
    ...
    if (requestComplete && error) {
      errorSection = (
        <p>An error occurred when searching with that image: {errorMessage}</p>
      );
    }

    let searchButton = (
      <Button variant="contained" onClick={this.findImageMatch}>
        Search for Image
      </Button>
    );
