...
function rootReducer(state = initialState, action: Action) {
    ...
    switch (action.type) {
        ...
        case ADD_CAPTURED_PHOTO:
            return { ...state, photoUri: action.payload.photoUri, photoCaptured: action.payload.photoCaptured }

        case CROP_PHOTO:
            return { ...state, croppedPhotoUri: action.payload.croppedPhotoUri, croppedPhotoBlob: action.payload.croppedPhotoBlob, croppedPhotoSet: action.payload.croppedPhotoSet }
